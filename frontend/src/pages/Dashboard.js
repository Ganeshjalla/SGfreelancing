...
import { FolderOpen, Gavel, CreditCard, TrendingUp, Plus, ArrowRight, Clock, CheckCircle } from 'lucide-react';
  ...
  return (
    <div className="card" style={{ padding: 24, animation: `slideUp 0.5s ease ${delay}s both` }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
        ...
        <div style={{ width: 48, height: 48, borderRadius: 14, background: color + '20', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
...
export default function Dashboard() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      if (user?.role === 'ADMIN') {
        const [statsRes, projRes] = await Promise.all([adminAPI.getStats(), adminAPI.getProjects()]);
        setStats(statsRes.data);
        setRecentProjects(projRes.data.slice(0, 5));
      } else if (user?.role === 'CLIENT') {
        const [myProj, myPay, profile] = await Promise.all([projectAPI.getMy(), paymentAPI.getMy(), userAPI.getProfile()]);
        ...
        const [myBids, myPay, profile] = await Promise.all([bidAPI.getMy(), paymentAPI.getMy(), userAPI.getProfile()]);
        updateUser({ walletBalance: profile.data.walletBalance });
        setStats({
  ...
  if (loading) return (
    <div>
      ...
      <div className="grid-4">{[1,2,3,4].map(i => <div key={i} className="skeleton card" style={{ height: 120 }} />)}</div>
    </div>
  );
  ...
  return (
    <div>
      <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        ...
        {user?.role === 'CLIENT' && <Link to="/create-project" className="btn btn-primary"><Plus size={16} /> Post Project</Link>}
        {user?.role === 'STUDENT' && <Link to="/projects" className="btn btn-primary"><FolderOpen size={16} /> Browse Projects</Link>}
      </div>
      ...
      {user?.role === 'CLIENT' && (
        <div className="card" style={{ padding: 24, marginBottom: 24, background: 'linear-gradient(135deg, #6366f1, #8b5cf6)', color: '#fff', animation: 'slideUp 0.5s ease 0.4s both' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <Link to="/transactions" style={{ background: 'rgba(255,255,255,0.2)', padding: '10px 20px', borderRadius: 12, color: '#fff', textDecoration: 'none', fontWeight: 600, fontSize: 14, display: 'flex', alignItems: 'center', gap: 6 }}>
      ...
      <div className="card" style={{ padding: 24, animation: 'slideUp 0.5s ease 0.5s both' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <h2 style={{ fontSize: 18, fontWeight: 700 }}>{user?.role === 'STUDENT' ? 'Recent Bids' : 'Recent Projects'}</h2>
        ...
        {recentProjects.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {recentProjects.map((item, i) => (
              <Link key={item.id} to={user?.role === 'STUDENT' ? `/projects/${item.projectId || item.id}` : `/projects/${item.id}`} style={{ textDecoration: 'none' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px', borderRadius: 12, background: '#f8fafc', border: '1px solid var(--border)', transition: 'all 0.2s', animation: `slideUp 0.4s ease ${0.1 * i}s both` }}
                  ...
                  <div>
                    <p style={{ fontWeight: 600, fontSize: 14, color: 'var(--text)' }}>{item.title || item.projectTitle}</p>
