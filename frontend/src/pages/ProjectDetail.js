...
import { Clock, DollarSign, Gavel, CheckCircle, XCircle, Upload, Link as LinkIcon, Star } from 'lucide-react';
...
export default function ProjectDetail() {
  ...
  const handleBid = async (e) => {
    ...
    try {
      ...
      loadAll();
    } catch (err) { setBidError(err.response?.data?.message || 'Failed to place bid'); }
    finally { setBidLoading(false); }
  };

  const handleAcceptBid = async (bidId) => {
    try { await bidAPI.accept(bidId); showSuccess('Bid accepted! Project is now in progress.'); loadAll(); }
    catch (err) { alert(err.response?.data?.message || 'Failed to accept bid'); }
  };

  const handleRejectBid = async (bidId) => {
    try { await bidAPI.reject(bidId); loadAll(); }
  ...
  const handleSubmitWork = async (e) => {
    ...
    try {
      ...
      loadAll();
    } catch (err) { setSubError(err.response?.data?.message || 'Failed to submit work'); }
    finally { setSubLoading(false); }
  };

  const handleReviewSubmission = async (subId, status, feedback = '') => {
    try { await submissionAPI.review(subId, { status, feedback }); showSuccess(`Submission ${status.toLowerCase()}!`); loadAll(); }
    catch (err) { alert(err.response?.data?.message || 'Failed to review submission'); }
  };
    ...
    <div style={{ maxWidth: 900, margin: '0 auto' }}>
      ...
      <div className="card" style={{ padding: 28, marginBottom: 24, animation: 'slideUp 0.4s ease' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
          <div style={{ flex: 1 }}>
            <span className={`badge badge-${project.status?.toLowerCase()}`} style={{ marginBottom: 12 }}>{project.status?.replace('_', ' ')}</span>
            <h1 style={{ fontSize: 24, fontWeight: 800, marginBottom: 8 }}>{project.title}</h1>
            <p style={{ color: 'var(--text-muted)', fontSize: 14 }}>Posted by <strong>{project.clientName}</strong> · {new Date(project.createdAt).toLocaleDateString()}</p>
          </div>
        ...
        <div style={{ display: 'flex', gap: 20, marginTop: 20, flexWrap: 'wrap' }}>
          {project.category && <span style={{ fontSize: 13, color: 'var(--text-muted)' }}>📁 {project.category}</span>}
          {project.deadline && <span style={{ fontSize: 13, color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: 6 }}><Clock size={14} /> Due {new Date(project.deadline).toLocaleDateString()}</span>}
          <span style={{ fontSize: 13, color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: 6 }}><Gavel size={14} /> {project.bidCount || bids.length} bids</span>
        </div>
        {project.requiredSkills && (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 16 }}>
            {project.requiredSkills.split(',').filter(Boolean).map(s => (
              <span key={s} style={{ padding: '4px 12px', background: 'var(--primary-light)', color: 'var(--primary)', borderRadius: 20, fontSize: 12, fontWeight: 600 }}>{s.trim()}</span>
            ))}
          </div>
        )}
      ...
      <div style={{ display: 'flex', gap: 4, marginBottom: 20, background: '#fff', padding: 6, borderRadius: 12, border: '1px solid var(--border)' }}>
        {tabs.map(t => (
          <button key={t.key} onClick={() => setTab(t.key)} style={{ flex: 1, padding: '10px 16px', border: 'none', borderRadius: 8, background: tab === t.key ? 'var(--primary)' : 'transparent', color: tab === t.key ? '#fff' : 'var(--text-muted)', fontWeight: 600, fontSize: 14, cursor: 'pointer', transition: 'all 0.2s', fontFamily: 'Inter, sans-serif' }}>
            {t.label}
          </button>
        ))}
      </div>
              ...
              <form onSubmit={handleBid}>
                <div className="grid-2" style={{ marginBottom: 16 }}>
                  <div>
                    ...
                    <input className="input" type="number" placeholder="e.g. 5000" required min="1" value={bidForm.amount} onChange={e => setBidForm({ ...bidForm, amount: e.target.value })} />
                  </div>
                  <div>
                    <label className="label">Delivery Days</label>
                    <input className="input" type="number" placeholder="e.g. 7" min="1" value={bidForm.deliveryDays} onChange={e => setBidForm({ ...bidForm, deliveryDays: e.target.value })} />
                  </div>
                </div>
                <div style={{ marginBottom: 16 }}>
                  <label className="label">Your Proposal</label>
                  <textarea className="input" rows={4} placeholder="Explain why you're the best fit..." required value={bidForm.proposal} onChange={e => setBidForm({ ...bidForm, proposal: e.target.value })} />
                </div>
            ...
            <div style={{ marginTop: 28, padding: 24, background: 'var(--primary-light)', borderRadius: 16, border: '1px solid var(--primary)' }}>
              ...
              <form onSubmit={handleSubmitWork}>
                <div style={{ marginBottom: 14 }}>
                  ...
                  <textarea className="input" rows={4} placeholder="Describe what you've built..." required value={subForm.description} onChange={e => setSubForm({ ...subForm, description: e.target.value })} />
                </div>
                <div className="grid-2" style={{ marginBottom: 14 }}>
                  <div>
                    <label className="label">GitHub Repository URL</label>
                    <input className="input" type="url" placeholder="https://github.com/..." value={subForm.githubUrl} onChange={e => setSubForm({ ...subForm, githubUrl: e.target.value })} />
                  </div>
                  <div>
                    <label className="label">Live Demo URL</label>
                    <input className="input" type="url" placeholder="https://yourapp.com" value={subForm.liveUrl} onChange={e => setSubForm({ ...subForm, liveUrl: e.target.value })} />
                  </div>
                </div>
      ...
      {tab === 'bids' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16, animation: 'fadeIn 0.3s ease' }}>
          {bids.length === 0 ? (
            <div className="empty-state card" style={{ padding: 48 }}><Gavel size={48} /><h3>No bids yet</h3><p>Bids from students will appear here</p></div>
          ) : bids.map(bid => (
            <div key={bid.id} className="card" style={{ padding: 24 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12, flexWrap: 'wrap', gap: 12 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <div style={{ width: 42, height: 42, borderRadius: '50%', background: 'var(--primary-light)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--primary)', fontWeight: 700, fontSize: 16 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                {isClient && project.status === 'OPEN' && bid.status === 'PENDING' && (
                  <div style={{ display: 'flex', gap: 8 }}>
                    <button className="btn btn-success btn-sm" onClick={() => handleAcceptBid(bid.id)}><CheckCircle size={14} /> Accept</button>
                    <button className="btn btn-danger btn-sm" onClick={() => handleRejectBid(bid.id)}><XCircle size={14} /> Reject</button>
                  </div>
                )}
              </div>
            </div>
      ...
      {tab === 'submissions' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16, animation: 'fadeIn 0.3s ease' }}>
          {submissions.length === 0 ? (
            <div className="empty-state card" style={{ padding: 48 }}><Upload size={48} /><h3>No submissions yet</h3></div>
          ) : submissions.map(sub => (
            <div key={sub.id} className="card" style={{ padding: 24 }}>
              ...
              <div style={{ display: 'flex', gap: 12, marginBottom: 16, flexWrap: 'wrap' }}>
                {sub.githubUrl && <a href={sub.githubUrl} target="_blank" rel="noopener noreferrer" className="btn btn-outline btn-sm"><LinkIcon size={13} /> GitHub</a>}
                {sub.liveUrl && <a href={sub.liveUrl} target="_blank" rel="noopener noreferrer" className="btn btn-outline btn-sm"><LinkIcon size={13} /> Live Demo</a>}
              </div>
              {sub.feedback && <div style={{ padding: 12, background: '#f8fafc', borderRadius: 10, marginBottom: 12, fontSize: 13 }}><strong>Feedback:</strong> {sub.feedback}</div>}
              {isClient && sub.status === 'PENDING' && (
                <div style={{ display: 'flex', gap: 8 }}>
                  <button className="btn btn-success btn-sm" onClick={() => handleReviewSubmission(sub.id, 'APPROVED', 'Great work!')}><CheckCircle size={14} /> Approve</button>
                  <button className="btn btn-ghost btn-sm" onClick={() => { const fb = prompt('Enter revision feedback:'); if (fb) handleReviewSubmission(sub.id, 'REVISION_REQUESTED', fb); }}>Request Revision</button>
                  <button className="btn btn-danger btn-sm" onClick={() => handleReviewSubmission(sub.id, 'REJECTED', 'Does not meet requirements')}><XCircle size={14} /> Reject</button>
                </div>
              )}
            </div>
          ))}
